...
render::render()
    : WinWindow(L"Lab2_ComputerGraphicsSamoilenko", 800, 600, false)
{
}
...
D2D1_POINT_2F render::ComputePoint(float t){
...
void render::Translate(float dx, float dy) {
    for (auto& point : controlPoints) {
        point.x += dx;
        point.y += dy;
    }
...
int render::BinomialCoefficient(int n, int k) {
    int result = 1;
    for (int i = 1; i <= k; i++) {
        result *= (n - i + 1);
        result /= i;
